...
public class ResideLayout extends ViewGroup {
     ...
     */
//    private static final int DEFAULT_OVERHANG_SIZE = 140; // dp;
     ...
     */
//    private static final int MIN_FLING_VELOCITY = 140; // dips per second
    ...
    public ResideLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (SystemUtility.isTablet(context)) {
            mOverhangSize = (int) (420 * density + 0.5f);
        } else {
            mOverhangSize = (int) (140 * density + 0.5f);
        }
...
//        mOverhangSize = (int) (DEFAULT_OVERHANG_SIZE * density + 0.5f);
        ...
        if (SystemUtility.isTablet(context)) {
            mDragHelper.setMinVelocity(420 * density);
        } else {
            mDragHelper.setMinVelocity(140 * density);
        }
...
//        MyLog.Set("d", ResideLayout.class, "mSlideOffset => " + mSlideOffset);
        if (vAlpha != null) {
            vAlpha.setAlpha(mSlideOffset);
...
//        }
    ...
    protected boolean drawChild(@NonNull Canvas canvas, @NonNull View child, long drawingTime) {
        ...
        // py => 前面的view高度位置變化　
        // default => getHeight()/2
        // app目前設置 getHeight() * 1.2f



        float py, upview_sx, upview_sy;
        if (SystemUtility.isTablet(mContext)) {
            py = getHeight() * 1.1f;

            upview_sx = 1 - mSlideOffset / 6;
            upview_sy = 1 - mSlideOffset / 6;

        } else {
            py = getHeight() * 1.2f;

            upview_sx = 1 - mSlideOffset / 3;
            upview_sy = 1 - mSlideOffset / 3;
        ...
        if (mCanSlide && !lp.slideable && mSlideableView != null) {
            ...
            canvas.scale(upview_sx,
                    upview_sy,
                    mSlideableView.getLeft(),
                    py);

//            ViewCompat.setRotationY(child, -10 * mSlideOffset); //Z軸
        }
