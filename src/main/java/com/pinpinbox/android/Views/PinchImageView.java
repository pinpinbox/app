...
//        float displayWidth = getMeasuredWidth();
//        float displayHeight = getMeasuredHeight();
//        scale(new PointF(displayWidth/2 ,displayHeight/2), 0.001572f, 590, new PointF(displayWidth/2 ,displayHeight/2));
//    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            MyLog.Set("e",PinchImageView.class, "event.getX() => " +  event.getX());

            MyLog.Set("e",PinchImageView.class, "event.getY() => " +  event.getY());
            ...
            if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                ...
                if (mPinchMode == PINCH_MODE_SCROLL) {
                    ...
                    //处理缩放
                    MyLog.Set("d",PinchImageView.class, "mScaleCenter => " + mScaleCenter);
                    MyLog.Set("d",PinchImageView.class, "mScaleBase => " + mScaleBase);
                    MyLog.Set("d",PinchImageView.class, "distance => " + distance);
                    MyLog.Set("d",PinchImageView.class, "mLastMovePoint => " + mLastMovePoint);
     ...
     */
    public void scale(PointF scaleCenter, float scaleBase, float distance, PointF lineCenter) {
        if (getDrawable() == null) {
            return;
        }
        //计算第二层缩放值
